...
const SLIME_BASIC_TEXTURE := preload("res://images/slime_basic.png")
const SLIME_DANGER_TEXTURE := preload("res://images/slime_danger.png")
const SLIME_HURT_TEXTURE := preload("res://images/slime_hurt.png")
...
const RECEPTOR_Y := 548.0
const SPACING := 82.0
const SCROLL := 520.0                 # px per second (내려오는 속도)
const P1_BASE_X := 71.0
const P2_BASE_X := 963.0
const ARROW_SIZE := 72.0              # 레인 화살표(리셉터)와 내려오는 화살표(노트) 공용 크기
const NOTE_OUTLINE_RADIUS := 12
const NOTE_BOUNCE_DURATION := 0.18
const NOTE_BOUNCE_SCALE := 0.34
const SLIME_DANGER_THRESHOLD := 0.30
const SLIME_HURT_DURATION := 0.11
const SLIME_SHAKE_AMPLITUDE := 15.0
const SLIME_SHAKE_STEP_DURATION := 0.018
const SLIME_SHAKE_RETURN_DURATION := 0.022
const SLIME_HIT_DARK_COLOR := Color(0.72, 0.72, 0.72, 1.0)
...
# 튜토리얼에서 사용 중인 흑백 상태바를 프로토타입에서도 그대로 쓴다.
class StatusBar:
	extends Control
	...
	func setup(status_font: Font, text_size: int) -> void:
		mouse_filter = Control.MOUSE_FILTER_IGNORE
		base_label = _make_label(status_font, text_size, Color.WHITE)
		add_child(base_label)
		fill_clip = Control.new()
		fill_clip.mouse_filter = Control.MOUSE_FILTER_IGNORE
		fill_clip.clip_contents = true
		add_child(fill_clip)
		fill_label = _make_label(status_font, text_size, Color.BLACK)
	...
	func _make_label(status_font: Font, text_size: int, color: Color) -> Label:
		var new_label := Label.new()
		new_label.add_theme_font_override("font", status_font)
...
func _ready() -> void:
	...
	slime_fx_rng.randomize()
	enemy_base_position = enemy_visual.position
	_update_slime_visual(0.0)
...
func _make_status_bar(pos: Vector2, bar_size: Vector2, font_size: int) -> StatusBar:
	...
	status_bar.setup(UI_FONT, font_size)
	return status_bar


func _set_status_bars_visible(show_bars: bool) -> void:
	for status_bar in [boss_hp_bar, gauge_bar, p1_hp_bar, p2_hp_bar]:
		if status_bar:
			status_bar.visible = show_bars


func _sync_status_bars() -> void:
	if not boss_hp_bar:
		return
	var monster_value := int(round(monster_hp * 200.0))
	var p1_value := int(round(p1_hp * 50.0))
	var p2_value := int(round(p2_hp * 50.0))
	var gauge_value := int(round(shared_gauge * 100.0))
	boss_hp_bar.set_stats(float(monster_value), 200.0, "HP %d/200" % monster_value)
	p1_hp_bar.set_stats(float(p1_value), 50.0, "HP %d/50" % p1_value)
	p2_hp_bar.set_stats(float(p2_value), 50.0, "HP %d/50" % p2_value)
	gauge_bar.set_stats(float(gauge_value), 100.0, "게이지 %d/100" % gauge_value)
...
func _setup_audio() -> void:
	...
	flick_stream = _load("res://audio/flick.wav")
	longnote_stream = _load("res://audio/longnote.wav")
	perfect_stream_p1 = _load("res://audio/perfect_p1.wav")
	good_stream_p1 = _load("res://audio/good_p1.wav")
	miss_stream_p1 = _load("res://audio/miss_p1.wav")
	flick_stream_p1 = _load("res://audio/flick_p1.wav")
	longnote_stream_p1 = _load("res://audio/longnote_p1.wav")
	longnote_player_p1 = AudioStreamPlayer.new(); add_child(longnote_player_p1)
	longnote_player_p2 = AudioStreamPlayer.new(); add_child(longnote_player_p2)
	longnote_player_p1.bus = &"SFX"
	longnote_player_p2.bus = &"SFX"
	_configure_wav_loop(longnote_stream)
	_configure_wav_loop(longnote_stream_p1)
	if longnote_stream:
		longnote_player_p1.stream = longnote_stream
	if longnote_stream_p1:
		longnote_player_p2.stream = longnote_stream_p1
	for i in range(10):                # 재사용 풀
...
# 같은 판정이라도 P1/P2 사운드가 겹치지 않도록 플레이어 풀에서 순환 사용한다.
func _play_hit_sound(kind: String, side: int) -> void:
	var p: AudioStreamPlayer = tone_players[tone_idx]
	tone_idx = (tone_idx + 1) % tone_players.size()
	var is_p1: bool = side == 0
	var stream: AudioStream = miss_stream if is_p1 else miss_stream_p1
	if kind == "perfect":
		stream = perfect_stream if is_p1 else perfect_stream_p1
	elif kind == "good":
		stream = good_stream if is_p1 else good_stream_p1
	elif kind == "flick":
		stream = flick_stream if is_p1 else flick_stream_p1
	if stream:
		p.volume_db = FLICK_VOLUME_DB if kind == "flick" else 0.0
...
func _process(delta: float) -> void:
	...
	# 이펙트 감쇠
	for i in range(8):
		receptor_flash[i] = max(0.0, receptor_flash[i] - delta / NOTE_BOUNCE_DURATION)
	for burst in hit_note_bursts:
		burst.life -= delta
	hit_note_bursts = hit_note_bursts.filter(func(burst): return burst.life > 0.0)
	for pu in popups:
		pu.life -= delta
...
func _trigger_slime_hurt() -> void:
	slime_hurt_timer = SLIME_HURT_DURATION
	_update_slime_visual(0.0)
	if slime_shake_tween and slime_shake_tween.is_valid():
		slime_shake_tween.kill()
	if slime_flash_tween and slime_flash_tween.is_valid():
		slime_flash_tween.kill()
	enemy_visual.position = enemy_base_position + _random_slime_shake_offset(1.0)
	enemy_visual.self_modulate = SLIME_HIT_DARK_COLOR
	slime_shake_tween = create_tween()
	slime_shake_tween.set_trans(Tween.TRANS_LINEAR)
	for strength in [1.0, 0.85, 0.68, 0.5]:
		slime_shake_tween.tween_property(
			enemy_visual,
			"position",
			enemy_base_position + _random_slime_shake_offset(strength),
			SLIME_SHAKE_STEP_DURATION
		)
	slime_shake_tween.tween_property(enemy_visual, "position", enemy_base_position, SLIME_SHAKE_RETURN_DURATION)
	slime_flash_tween = create_tween()
	slime_flash_tween.tween_interval(0.018)
	slime_flash_tween.set_trans(Tween.TRANS_QUAD)
	slime_flash_tween.set_ease(Tween.EASE_OUT)
	slime_flash_tween.tween_property(enemy_visual, "self_modulate", Color.WHITE, 0.052)


func _random_slime_shake_offset(strength: float) -> Vector2:
	var direction := Vector2.from_angle(slime_fx_rng.randf_range(0.0, TAU))
	var distance := SLIME_SHAKE_AMPLITUDE * strength * slime_fx_rng.randf_range(0.72, 1.0)
	return direction * distance


func _update_slime_visual(delta: float) -> void:
	slime_hurt_timer = max(0.0, slime_hurt_timer - delta)
	var desired_texture: Texture2D
	if slime_hurt_timer > 0.0:
		desired_texture = SLIME_HURT_TEXTURE
	elif monster_hp <= SLIME_DANGER_THRESHOLD:
		desired_texture = SLIME_DANGER_TEXTURE
	else:
		desired_texture = SLIME_BASIC_TEXTURE
	if enemy_visual.texture != desired_texture:
		enemy_visual.texture = desired_texture
...
func _update_side(t: float, side: int, pressed: Array, released: Array, lane_held_side: Array) -> void:
	...
	for i in range(4):
		...
		if best.type == "hold":
			best.state = "holding"
			_judge_head(best_dt, rec_base + i, side, best.type)
		else:  # tap / slur
			best.state = "done"
			_judge_head(best_dt, rec_base + i, side, best.type)
...
func _judge_head(dt: float, rec: int, side: int, note_type: String = "tap") -> void:
	if dt <= W_PERFECT: _hit("PERFECT", rec, side, note_type)
	elif dt <= W_GOOD:  _hit("GOOD", rec, side, note_type)
	else:               _hit("GOOD", rec, side, note_type)   # W_MISS 안이면 최소 GOOD
...
func _spawn_hit_note_burst(rec: int) -> void:
	hit_note_bursts.append({
		"side": 0 if rec < 4 else 1,
		"lane": rec % 4,
		"life": NOTE_BOUNCE_DURATION,
	})
...
func _activate_slur_sustain(head: Note) -> void:
	...
	if chain_end > head.time:
		if head.side == 0:
			slur_sustain_end_times_p1.append(chain_end)
		else:
			slur_sustain_end_times_p2.append(chain_end)


func _refresh_longnote_loop(t: float) -> void:
	for index in range(slur_sustain_end_times_p1.size() - 1, -1, -1):
		if t > slur_sustain_end_times_p1[index]:
			slur_sustain_end_times_p1.remove_at(index)
	for index in range(slur_sustain_end_times_p2.size() - 1, -1, -1):
		if t > slur_sustain_end_times_p2[index]:
			slur_sustain_end_times_p2.remove_at(index)
	var should_loop_p1: bool = not slur_sustain_end_times_p1.is_empty()
	var should_loop_p2: bool = not slur_sustain_end_times_p2.is_empty()
	...
	for nt in notes:
		...
		if lane_is_held:
			var hold_volume := _longnote_volume_for_note(nt, t)
			if nt.side == 0:
				should_loop_p1 = true
				...
				has_hold_p2 = true
	if should_loop_p1:
		longnote_player_p1.volume_db = hold_volume_p1
		if longnote_player_p1.stream != null and not longnote_player_p1.playing:
			longnote_player_p1.play(0.0)
	elif longnote_player_p1.playing:
		longnote_player_p1.stop()
	if should_loop_p2:
		longnote_player_p2.volume_db = hold_volume_p2
		if longnote_player_p2.stream != null and not longnote_player_p2.playing:
			longnote_player_p2.play(0.0)
	elif longnote_player_p2.playing:
		longnote_player_p2.stop()


func _longnote_volume_for_note(nt: Note, t: float) -> float:
	var duration: float = max(nt.length, 0.01)
	var progress: float = clamp((t - nt.time) / duration, 0.0, 1.0)
	return lerp(LONGNOTE_START_VOLUME_DB, LONGNOTE_END_VOLUME_DB, progress)


func _popup(txt: String, rec: int, col: Color) -> void:
	var side := 0 if rec < 4 else 1
	var x := _lane_x(side, rec % 4)
...
func _end_song() -> void:
	...
	finished = true
	backing.stop()
	if longnote_player_p1.playing:
		longnote_player_p1.stop()
	if longnote_player_p2.playing:
		longnote_player_p2.stop()
...
# ============================================================
func _draw() -> void:
	# 배경/적/P1/P2 이미지는 Main.tscn의 Visuals CanvasLayer가 담당한다.
	# 리셉터
	for i in range(4):
		_draw_receptor(_lane_x(0, i), i, receptor_flash[i], false)
		_draw_receptor(_lane_x(1, i), i, receptor_flash[4 + i], true)
...
func _draw_receptor(x: float, lane: int, flash: float, is_p2: bool) -> void:
	...
	var bounce_size: float = ARROW_SIZE * (1.0 + NOTE_BOUNCE_SCALE * flash)
	_draw_arrow(Vector2(x, RECEPTOR_Y), lane, c, bounce_size)


func _lane_x(side: int, lane: int) -> float:
	return (P1_BASE_X if side == 0 else P2_BASE_X) + lane * SPACING


func _draw_note(nt: Note) -> void:
	if nt.state in ["done", "missed"]:
		return
	var x := _lane_x(nt.side, nt.lane)
	var y := RECEPTOR_Y + ((song_time - AUDIO_OFFSET) - nt.time) * SCROLL
	...
	if nt.type == "hold":
		var y_tail_end := RECEPTOR_Y + ((song_time - AUDIO_OFFSET) - nt.time - nt.length) * SCROLL
		...
		_draw_chart_arrow(Vector2(x, y_head), nt.lane, ARROW_SIZE)
		return
	...
	_draw_chart_arrow(Vector2(x, y), nt.lane, ARROW_SIZE)
	if nt.type == "rapid":
...
func _draw_slur_connections() -> void:
	...
	for nt in notes:
		...
		var start: Vector2 = Vector2(_lane_x(nt.side, nt.lane), RECEPTOR_Y + (chart_time - nt.time) * SCROLL)
		var finish: Vector2 = Vector2(_lane_x(next_note.side, next_note.lane), RECEPTOR_Y + (chart_time - next_note.time) * SCROLL)
		var col: Color = LANE_COLORS[nt.lane]
		if nt.side == 0:
			col = col.darkened(0.15)
		...
		var trail_start: Vector2 = Vector2(_lane_x(nt.side, nt.lane), RECEPTOR_Y)
		var trail_finish: Vector2 = Vector2(_lane_x(next_note.side, next_note.lane), RECEPTOR_Y - link_duration * SCROLL)
...
func _arrow_body_width(lane: int, size_value: float) -> float:
	return size_value * 0.38


# 고정 리셉터: 검정 내부와 방향별 색 테두리를 유지한다.
func _draw_arrow(pos: Vector2, lane: int, col: Color, size_value: float) -> void:
	if lane < 0 or lane >= NOTE_TEXTURES.size():
		return
	var texture: Texture2D = NOTE_TEXTURES[lane]
	var rect := Rect2(pos - Vector2(size_value, size_value) * 0.5, Vector2(size_value, size_value))
	if lane < note_outline_textures.size():
		draw_texture_rect(note_outline_textures[lane], rect, false, Color.WHITE)
	draw_texture_rect(texture, rect, false, col)


# 내려오는 채보: 흰색 내부와 방향별 색 테두리로 반전한 텍스처를 쓴다.
func _draw_chart_arrow(pos: Vector2, lane: int, size_value: float, alpha: float = 1.0) -> void:
	if lane < 0 or lane >= chart_note_textures.size():
		return
	var texture: Texture2D = chart_note_textures[lane]
	var rect := Rect2(pos - Vector2(size_value, size_value) * 0.5, Vector2(size_value, size_value))
	if lane < note_outline_textures.size():
		draw_texture_rect(note_outline_textures[lane], rect, false, Color(0.0, 0.0, 0.0, alpha))
	draw_texture_rect(texture, rect, false, Color(1.0, 1.0, 1.0, alpha))


func _draw_hit_note_bursts() -> void:
	for burst in hit_note_bursts:
		var life: float = float(burst.life)
		var progress: float = clamp(1.0 - life / NOTE_BOUNCE_DURATION, 0.0, 1.0)
		var bounce_scale: float = 1.0 + NOTE_BOUNCE_SCALE * sin(progress * PI)
		var alpha: float = clamp(life / (NOTE_BOUNCE_DURATION * 0.35), 0.0, 1.0)
		var side: int = int(burst.side)
		var lane: int = int(burst.lane)
		_draw_chart_arrow(Vector2(_lane_x(side, lane), RECEPTOR_Y), lane, ARROW_SIZE * bounce_scale, alpha)
